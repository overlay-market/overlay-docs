...
## Introduction
At the protocol level, Overlay uses OVL to settle PnL — minting OVL to cover trader profits and burning OVL to absorb losses. While traders interact entirely in stablecoins, this mechanism means the protocol is exposed to potential OVL inflation risk, particularly in markets based on long-tail assets.
